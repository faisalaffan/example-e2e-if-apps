...
methods.initBundletool = async function initBundletool() {
    try {
        this.binaries.bundletool = await support_1.fs.which('bundletool.jar');
    }
    catch (err) {
...
methods.setHiddenApiPolicy = async function setHiddenApiPolicy(value, ignoreError = false) {
    try {
        ...
        logger_js_1.default.info(`Failed to set setting keys '${HIDDEN_API_POLICY_KEYS}' to '${value}'. Original error: ${e.message}`);
    }
};
/**
...
methods.setDefaultHiddenApiPolicy = async function setDefaultHiddenApiPolicy(ignoreError = false) {
    try {
        ...
        logger_js_1.default.info(`Failed to delete keys '${HIDDEN_API_POLICY_KEYS}'. Original error: ${e.message}`);
    }
};
/**
...
methods.broadcastAirplaneMode = async function broadcastAirplaneMode(on) {
    ...
    try {
        ...
        if (lodash_1.default.includes(e.stderr, 'SecurityException')) {
            try {
    ...
    if (await this.getApiLevel() >= 23) {
        ...
        if (this._isPgrepAvailable || this._isPidofAvailable) {
            ...
            try {
                ...
                if (lodash_1.default.includes(e.stderr || e.stdout, 'syntax error')) {
                    logger_js_1.default.warn(`Got an unexpected response from the shell interpreter: ${e.stderr || e.stdout}`);
                }
                else {
                    return [];
                }
...
methods.killProcessByPID = async function killProcessByPID(pid) {
    ...
    try {
        ...
        if (lodash_1.default.includes(e.stderr, noProcessFlag)) {
            return;
        }
        if (!lodash_1.default.includes(e.stderr, 'Operation not permitted')) {
            throw e;
        }
        ...
        try {
            ...
            if (lodash_1.default.includes(e1.stderr, noProcessFlag)) {
                return;
            }
            throw e1;
        }
...
methods.listFeatures = async function listFeatures() {
    ...
    try {
        ...
        if (lodash_1.default.includes(e.stderr, 'unknown command')) {
            return [];
        }
        throw e;
    }
