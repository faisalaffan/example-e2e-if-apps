...
apkSigningMethods.executeApksigner = async function executeApksigner(args = []) {
    ...
    logger_js_1.default.debug(`Starting apksigner: ${support_1.util.quote(fullCmd)}`);
    ...
    const { stdout, stderr } = await (0, teen_process_1.exec)(fullCmd[0], fullCmd.slice(1), {
        cwd: path_1.default.dirname(apkSignerJar), windowsVerbatimArguments: support_1.system.isWindows()
    });
    for (let [name, stream] of [['stdout', stdout], ['stderr', stderr]]) {
        ...
        logger_js_1.default.debug(`apksigner ${name}: ${stream}`);
    }
    return stdout;
};
/**
...
apkSigningMethods.signWithDefaultCert = async function signWithDefaultCert(apk) {
    logger_js_1.default.debug(`Signing '${apk}' with default cert`);
...
apkSigningMethods.signWithCustomCert = async function signWithCustomCert(apk) {
    logger_js_1.default.debug(`Signing '${apk}' with custom cert`);
    ...
    try {
        ...
        logger_js_1.default.warn(`Cannot use apksigner tool for signing. Defaulting to jarsigner. ` +
            `Original error: ${err.stderr || err.stdout || err.message}`);
        try {
            if (await (0, helpers_js_1.unsignApk)(apk)) {
                logger_js_1.default.debug(`'${apk}' has been successfully unsigned`);
            }
            else {
                logger_js_1.default.debug(`'${apk}' does not need to be unsigned`);
            }
            ...
            const fullCmd = [jarsigner,
                ...
                apk, this.keyAlias];
            logger_js_1.default.debug(`Starting jarsigner: ${support_1.util.quote(fullCmd)}`);
            await (0, teen_process_1.exec)(fullCmd[0], fullCmd.slice(1), { windowsVerbatimArguments: support_1.system.isWindows() });
        }
        catch (e) {
...
apkSigningMethods.zipAlignApk = async function zipAlignApk(apk) {
    ...
    try {
        ...
        logger_js_1.default.debug(`${apk}' is already zip-aligned. Doing nothing`);
        return false;
    }
    catch (e) {
        logger_js_1.default.debug(`'${apk}' is not zip-aligned. Aligning`);
    }
    try {
        await support_1.fs.access(apk, fs_1.default.W_OK);
    }
    catch (e) {
...
apkSigningMethods.checkApkCert = async function checkApkCert(appPath, pkg, opts = {}) {
    logger_js_1.default.debug(`Checking app cert for ${appPath}`);
    if (!await support_1.fs.exists(appPath)) {
        logger_js_1.default.debug(`'${appPath}' does not exist`);
        return false;
    }
    ...
    const hashMatches = (apksignerOutput, expectedHashes) => {
        for (const [name, value] of lodash_1.default.toPairs(expectedHashes)) {
            if (new RegExp(`digest:\\s+${value}\\b`, 'i').test(apksignerOutput)) {
                logger_js_1.default.debug(`${name} hash did match for '${path_1.default.basename(appPath)}'`);
                return true;
            }
        }
        return false;
    ...
    if (SIGNED_APPS_CACHE.has(appHash)) {
        logger_js_1.default.debug(`Using the previously cached signature entry for '${path_1.default.basename(appPath)}'`);
    ...
    try {
        ...
        if (hasMatch) {
            logger_js_1.default.info(`'${appPath}' is signed with the ` +
                `${this.useKeystore ? 'keystore' : 'default'} certificate`);
        }
        else {
            logger_js_1.default.info(`'${appPath}' is signed with a ` +
        ...
        if (lodash_1.default.includes(err.stderr, APKSIGNER_VERIFY_FAIL)) {
            logger_js_1.default.info(`'${appPath}' is not signed`);
            return false;
        }
        ...
        if (lodash_1.default.includes(errMsg, JAVA_PROPS_INIT_ERROR)) {
            ...
            logger_js_1.default.warn(errMsg);
            logger_js_1.default.warn(`Assuming '${appPath}' is already signed and continuing anyway`);
            return true;
        }
...
apkSigningMethods.getKeystoreHash = async function getKeystoreHash() {
    logger_js_1.default.debug(`Getting hash of the '${this.keystorePath}' keystore`);
    ...
    logger_js_1.default.info(`Running '${keytool}' with arguments: ${support_1.util.quote(args)}`);
    try {
        const { stdout } = await (0, teen_process_1.exec)(keytool, args, { windowsVerbatimArguments: support_1.system.isWindows() });
        const result = {};
        ...
        logger_js_1.default.debug(`Keystore hash: ${JSON.stringify(result)}`);
        return result;
    }
    catch (e) {
