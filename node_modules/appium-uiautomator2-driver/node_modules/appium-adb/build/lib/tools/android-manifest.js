...
manifestMethods.targetSdkVersionFromManifest = async function targetSdkVersionFromManifest(appPath) {
    ...
    const getTargetSdkViaAapt = async () => {
        ...
        const { stdout } = await (0, teen_process_1.exec)(this.binaries.aapt, args);
...
manifestMethods.compileManifest = async function compileManifest(manifest, manifestPackage, targetPackage) {
    const { platform, platformPath } = await (0, helpers_js_1.getAndroidPlatformAndPath)(this.sdkRoot);
    if (!platform) {
    ...
    try {
        ...
        logger_js_1.default.debug(`Compiling the manifest using '${support_1.util.quote([this.binaries.aapt2, ...args])}'`);
        await (0, teen_process_1.exec)(this.binaries.aapt2, args);
    }
    catch (e) {
        ...
        logger_js_1.default.debug(`Compiling the manifest using '${support_1.util.quote([this.binaries.aapt, ...args])}'`);
        try {
            await (0, teen_process_1.exec)(this.binaries.aapt, args);
        }
        catch (e1) {
...
manifestMethods.insertManifest = async function insertManifest(manifest, srcApk, dstApk) {
    ...
    try {
        ...
        try {
            await (0, teen_process_1.exec)(this.binaries.aapt, [
