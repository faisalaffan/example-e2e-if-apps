...
async function listEmulators() {
    ...
    if (avdsRoot) {
        logger_js_1.default.warn(`The value of the ANDROID_AVD_HOME environment variable '${avdsRoot}' is not an existing directory`);
    }
    ...
    if (!await (0, helpers_1.dirExists)(avdsRoot)) {
        logger_js_1.default.debug(`Virtual devices config root '${avdsRoot}' is not an existing directory`);
        return [];
    }
    return await getAvdConfigPaths(avdsRoot);
}
...
emuMethods.powerAC = async function powerAC(state = 'on') {
    if (lodash_1.default.values(emuMethods.POWER_AC_STATES).indexOf(state) === -1) {
        throw new TypeError(`Wrong power AC state sent '${state}'. `
            + `Supported values: ${lodash_1.default.values(emuMethods.POWER_AC_STATES)}]`);
    }
...
emuMethods.sensorSet = async function sensorSet(sensor, value) {
    if (!lodash_1.default.includes(emuMethods.SENSORS, sensor)) {
        throw new TypeError(`Unsupported sensor sent '${sensor}'. `
            + `Supported values: ${lodash_1.default.values(emuMethods.SENSORS)}]`);
    }
    if (lodash_1.default.isNil(value) || value === '') {
...
emuMethods.gsmCall = async function gsmCall(phoneNumber, action) {
    if (!lodash_1.default.values(emuMethods.GSM_CALL_ACTIONS).includes(action)) {
        throw new TypeError(`Invalid gsm action param ${action}. Supported values: ${lodash_1.default.values(emuMethods.GSM_CALL_ACTIONS)}`);
    }
    if (!lodash_1.default.isInteger(phoneNumber) && lodash_1.default.isEmpty(phoneNumber)) {
        throw new TypeError('Phone number most not be empty');
    }
...
emuMethods.gsmSignal = async function gsmSignal(strength = 4) {
    ...
    if (!emuMethods.GSM_SIGNAL_STRENGTHS.includes(strength)) {
        throw new TypeError(`Invalid signal strength param ${strength}. Supported values: ${lodash_1.default.values(emuMethods.GSM_SIGNAL_STRENGTHS)}`);
    }
    logger_js_1.default.info('gsm signal-profile <strength> changes the reported strength on next (15s) update.');
...
emuMethods.gsmVoice = async function gsmVoice(state = 'on') {
    ...
    if (!lodash_1.default.values(emuMethods.GSM_VOICE_STATES).includes(state)) {
        throw new TypeError(`Invalid gsm voice state param ${state}. Supported values: ${lodash_1.default.values(emuMethods.GSM_VOICE_STATES)}`);
    }
...
emuMethods.networkSpeed = async function networkSpeed(speed = 'full') {
    ...
    if (!lodash_1.default.values(emuMethods.NETWORK_SPEED).includes(speed)) {
        throw new Error(`Invalid network speed param ${speed}. Supported values: ${lodash_1.default.values(emuMethods.NETWORK_SPEED)}`);
    }
        ...
        client.on('data', (chunk) => {
            ...
            if (okFlag.test(output)) {
                ...
                if (!isCommandSent) {
                    ...
                    logger_js_1.default.debug(`Executing Emulator console command: ${cmdStr}`);
                    client.write(cmdStr);
                    client.write(eol);
                    isCommandSent = true;
...
emuMethods.getEmuVersionInfo = async function getEmuVersionInfo() {
    const propsPath = path_1.default.join(this.sdkRoot, 'emulator', 'source.properties');
...
emuMethods.getEmuImageProperties = async function getEmuImageProperties(avdName) {
    ...
    return ini_1.default.parse(await support_1.fs.readFile(avd.config, 'utf8'));
};
/**
 ...
 */
//# sourceMappingURL=adb-emu-commands.js.map