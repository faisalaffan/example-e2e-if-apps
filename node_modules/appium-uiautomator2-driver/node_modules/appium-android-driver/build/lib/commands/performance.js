...
exports.getBatteryInfo = exports.getCPUInfo = exports.getNetworkTrafficInfo = exports.getMemoryInfo = exports.NETWORK_KEYS = exports.BATTERY_KEYS = exports.MEMORY_KEYS = exports.CPU_KEYS = exports.SUPPORTED_PERFORMANCE_DATA_TYPES = exports.commands = void 0;
...
async function getMemoryInfo(packageName, retries = 2) {
    return await (0, asyncbox_1.retryInterval)(retries, RETRY_PAUSE_MS, async () => {
        ...
        if (valDict.totalPrivateDirty && valDict.totalPrivateDirty !== 'nodex') {
            const headers = lodash_1.default.clone(MEMORY_KEYS);
...
async function getNetworkTrafficInfo(retries = 2) {
    return await (0, asyncbox_1.retryInterval)(retries, RETRY_PAUSE_MS, async () => {
        ...
        if (!lodash_1.default.isEqual(pendingBytes, '') && !lodash_1.default.isUndefined(pendingBytes) && !lodash_1.default.isEqual(pendingBytes, 'nodex')) {
            return returnValue;
        }
        else {
...
async function getBatteryInfo(retries = 2) {
    return await (0, asyncbox_1.retryInterval)(retries, RETRY_PAUSE_MS, async () => {
        ...
        if (!Number.isNaN(power)) {
            return [lodash_1.default.clone(BATTERY_KEYS), [power.toString()]];
        }
        else {
...
commands.getPerformanceDataTypes = function getPerformanceDataTypes() {
    return lodash_1.default.keys(SUPPORTED_PERFORMANCE_DATA_TYPES);
};
/**
 * @returns The information type of the system state which is supported to read as like cpu, memory, network traffic, and battery.
...
commands.mobileGetPerformanceData = async function mobileGetPerformanceData(opts = {}) {
    const { packageName, dataType } = (0, utils_1.requireArgs)(['packageName', 'dataType'], opts);
